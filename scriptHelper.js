...
// validate each input according to assignment specifications (carLevel needs to be number, etc.)[ALERT], then validate each input is qualified (cargoLevel is > 1000, etc.)[innerHTML]
        ...
        document.getElementById("fuelStatus").innerHTML = `${fuelLevel.value}L is too low for the mission`;
        document.getElementById("launchStatus").innerHTML = "Shuttle not ready for launch";
        document.getElementById("launchStatus").style.color="red";
        }
        if (validateInput(cargoLevel.value) > 10000){
        document.getElementById("faultyItems").style="visibility: visible";
        document.getElementById("fuelStatus").innerHTML = `${fuelLevel.value}kg is too high for the mission`;
        document.getElementById("launchStatus").innerHTML = "Shuttle not ready for launch";
        document.getElementById("launchStatus").style.color="red";
